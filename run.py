...
def transaccion():
    ...
    if request.method == 'POST':
        ...
        r = requests.post('http://142.44.246.23:5596/coordinator', json=datos)
        re = r.json()
        ...
        if respuesta =="datos enviados al register": #Si es true los datos son correctos la transaccion es exitosa
            r = requests.post('http://142.44.246.23:5596/coordinator', json={"origen":"wallet","operacion":"consultarfondos"})
            re = r.json()
            respuesta = re['mensaje']
            print(respuesta)
